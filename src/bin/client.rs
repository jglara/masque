            ...
            tokio::select! {
                            ...
                            match http3_conn.poll(&mut conn) {
                                Ok((stream_id, quiche::h3::Event::Headers { list, .. })) => {
                                    ...
                                    if let Some(sender) = connect_streams.get(&stream_id) {
                                        sender.send(Content::Headers { stream_id, headers: list }).unwrap();
                                    }
                                },
                                        ...
                                        match http3_conn.recv_dgram(&mut conn, &mut buf) {
                                            Ok((read, flow_id, flow_id_len)) => {
                                                ...
                                                if let Some(sender) = connect_streams.get(&flow_id) {
                                                    sender.send(Content::Datagram { payload: buf[flow_id_len..read].to_vec() }).unwrap();
                                                } else {
                    ...
                    loop {
                        let result = match &to_send.content {
                            ...
                            Content::Finished => {
                                ...
                                conn.stream_shutdown(to_send.stream_id, quiche::Shutdown::Read, 0).unwrap();
                        ...
                        match result {
                            ...
                            Err(e) => {
                                ...
                                let _res = conn.stream_shutdown(to_send.stream_id, quiche::Shutdown::Read, 0);
                                let _res = conn.stream_shutdown(to_send.stream_id, quiche::Shutdown::Write, 0);
                ...
                _ = interval.tick(), if http3_conn.is_some() && http3_retry_send.is_some() => {
                    ...
                    match result {
                        ...
                        Err(e) => {
                            error!("Connection {} stream {} send failed {:?}", conn.trace_id(), to_send.stream_id, e);
                            conn.stream_shutdown(to_send.stream_id, quiche::Shutdown::Write, 0).unwrap();
                            {
                                connect_streams.remove(&to_send.stream_id);
                            }
                            http3_retry_send = None;
...
async fn handle_http1_stream(mut stream: TcpStream, http3_sender: UnboundedSender<ToSend>) {
    ...
    let _res = req.parse(&buf[..pos]).unwrap();
            ...
            if method.eq_ignore_ascii_case("CONNECT") {
                ...
                let headers = vec![
                    ...
                    quiche::h3::Header::new(
                        b":path",
                        format!("/.well_known/masque/udp/{}/{}/", host_port[0], host_port[1])
                            .as_bytes(),
                    ),
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        debug!(
                            ...
                            read, peer_addr, connect_stream_id
                        );
                        http3_sender_clone
                            .send(ToSend {
                                stream_id: connect_stream_id,
                                content: Content::Datagram {
                    ...
                    loop {
                        ...
                        match data {
                            ...
                            Content::Datagram { payload } => {
                                ...
                                debug!(
                                    "received UDP Proxying Datagram with context_id={:?} ",
                                    context_id
                                );
                                ...
                                while pos < payload.len() {
                                    let bytes_written: usize =
                                        match write_half.write(&payload[pos..]).await {
                                            Ok(v) => v,
                                            Err(e) => {
                                                error!(
                                                    "Error writing to TCP {} on stream id {}: {}",
                                                    peer_addr, connect_stream_id, e
                                                );
                                                return;
                                            }
                                        };
